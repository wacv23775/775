...
from .dsbn import DomainSpecificBatchNorm3d
...
def inflate_domain_specific_batch_norm(dsbatch2d):
    # In pytorch 0.2.0 the 2d and 3d versions of batch norm
    # work identically except for the check that verifies the
    # input dimensions
    ...
    #     batch3d = nn.BatchNorm3d(bn.num_features)
    #     bns.append(batch3d)
    # dsbatch2d.bns = nn.ModuleList(bns)
    # dsbatch2d._check_input_dim = batch3d._check_input_dim
    dsbatch2d = DomainSpecificBatchNorm3d(dsbatch2d.bns[0].num_features)
    return dsbatch2d
...
def inflate_domain_specific_to_3D_batch_norm(batch2d):
    # In pytorch 0.2.0 the 2d and 3d versions of batch norm
    # work identically except for the check that verifies the
    # input dimensions

    batch3d = nn.BatchNorm3d(batch2d.bns[0].num_features)
    # retrieve 3d _check_input_dim function
    batch2d._check_input_dim = batch3d._check_input_dim
    return batch2d