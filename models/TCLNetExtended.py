...
from .resnetdsbn import resnet50dsbn, TwoInputSequential
from .clone_2d import CloneBottleneck, CloneDomainSpecificBottleneck
from .TSEdsbn import TSEdsbn
from .TSB import TSB
...
class Bottleneck3d(nn.Module):
    ...
    def __init__(self, bottleneck2d, inflate_time=False):
        ...
        self.bn1 = inflate.inflate_domain_specific_batch_norm(bottleneck2d.bn1)
        self.conv2 = inflate.inflate_conv(bottleneck2d.conv2, time_dim=1)
        self.bn2 = inflate.inflate_domain_specific_batch_norm(bottleneck2d.bn2)
        self.conv3 = inflate.inflate_conv(bottleneck2d.conv3, time_dim=1)
        self.bn3 = inflate.inflate_domain_specific_batch_norm(bottleneck2d.bn3)
    ...
    def _inflate_downsample(self, downsample2d, time_stride=1):
        downsample3d = nn.Sequential(
            ...
            #inflate.inflate_domain_specific_to_3D_batch_norm(downsample2d[1]))
            inflate.inflate_batch_norm(nn.BatchNorm2d(downsample2d[1].bns[0].num_features)))
        return downsample3d

    def forward(self, x, domain_label):
        residual = x
        out = self.conv1(x)
        out, _ = self.bn1(out,domain_label=domain_label)
        out = self.relu(out)

        out = self.conv2(out)
        out, _ = self.bn2(out, domain_label=domain_label)
        out = self.relu(out)

        out = self.conv3(out)
        out, _ = self.bn3(out, domain_label=domain_label)
...
class TCLNetExtended(nn.Module):

    def __init__(self, num_classes, use_gpu, loss={'xent'}, backbone='resnet50'):
        super(TCLNetExtended, self).__init__()
        self.loss = loss
        self.use_gpu = use_gpu

        resnet2d = resnet50dsbn(pretrained=True)

        self.conv1 = inflate.inflate_conv(resnet2d.conv1, time_dim=1)
        self.bn1 = inflate.inflate_domain_specific_batch_norm(resnet2d.bn1)
        self.relu = nn.ReLU(inplace=True)
        ...
        layer4 = TwoInputSequential(resnet2d.layer4[0], resnet2d.layer4[1])

        branch = nn.ModuleList([CloneDomainSpecificBottleneck(resnet2d.layer4[-1]) for _ in range(2)])

        self.TSE_Module = TSEdsbn(layer4=layer4, branch=branch, use_gpu=use_gpu)
    ...
    def _inflate_reslayer(self, reslayer2d, enhance_idx=[], channels=0):
        ...
        return TwoInputSequential(*reslayers3d)
    ...
    def forward(self, x,domain_label):
        ...
        #x = self.layer1(x)
        x, _ = self.layer1(x, domain_label)  # inflate_reslayer(TSB)
        x = self.layer2(x, domain_label)
        x, _ = self.layer3(x, domain_label)
        b, c, t, h, w = x.size()

        x1_list = self.TSE_Module(x[:, :, :2], domain_label)
        x2_list = self.TSE_Module(x[:, :, 2:], domain_label)
