...
# python train.py --gpu_devices 0 --save-dir log-mars-tclnet --root /export/livia/home/vision/dmekhazni/KD-Reid/reid-data/mars -d mars
# python train.py --gpu_devices 0 --save-dir log-ilidsvid-tclnet --root /export/livia/home/vision/dmekhazni/KD-Reid/reid-data/ilids-vid -d ilids-vid
...
from losses import TripletLoss, SupConLoss, SupervisedContrastiveLoss
...
parser.add_argument('-d', '--dataset', type=str, default='ilids-vid',
                    choices=data_manager.get_names())
parser.add_argument('-dt', '--dataset_target', type=str, default='mars',
                    choices=data_manager.get_names())
...
parser.add_argument('--save-dir', type=Path, default=Path('/export/livia/home/vision/dmekhazni/VideoReID-TCLNet/log'))
...
def main():
    ...
    if args.loss_co == 'triplet':
        ...
        print("triplet")
    elif args.loss_co == 'SupCon':
        criterion_htri = SupConLoss()
        print("SupCon")
    ...
    with torch.no_grad():
        rank1 = test(model, queryloader_target, galleryloader_target, use_gpu)
    ...
    for epoch in range(start_epoch, args.max_epoch):
        ...
        if (epoch + 1) >= args.start_eval and (epoch + 1) % args.eval_step == 0 or epoch == 0:
            ...
            print("==> Test Target")
            with torch.no_grad():
                rank1_target = test(model, queryloader_target, galleryloader_target, use_gpu)
            ...
            if is_best:
                ...
                if use_gpu:
                    state_dict_best = model.module.state_dict()
                else:
        ...
        save_checkpoint({
            'state_dict': state_dict_best,
            'rank1': best_rank1,
            'epoch': best_epoch,
        }, osp.join(args.save_dir, 'best_at_epoch' + str(epoch + 1) + '.pth.tar'))
...
def train(epoch, model, criterion_xent, criterion_htri, optimizer, trainloader, use_gpu):
    ...
    for batch_idx, (vids, pids, camids, _, _) in enumerate(trainloader):
        ...
        for i in range(len(outputs)):
            ...
            # htri_loss += criterion_htri(input, pids)

            features_norm = model.module.bn[i](features[i])  # [bs, c]
            features_norm = F.normalize(features[i], p=2, dim=1, eps=1e-12)
            input = torch.reshape(features_norm, (
            int(features_norm.shape[0] / trainloader.sampler.num_instances), trainloader.sampler.num_instances,
            features_norm.shape[-1]))

            htri_loss += criterion_htri(input)
