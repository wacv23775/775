...
import urllib
import tarfile
from data_manager.utils import mkdir_if_missing, write_json, read_json
import json
...
class iLIDSVID(object):
    ...
    def __init__(self, root='/data/datasets/', split_id=9):
        self.name = 'ilids-vid'
        self.path = osp.join(root, self.name)
        self.root = osp.join(self.path, 'i-LIDS-VID')
        #self.root = osp.join(self.path)
